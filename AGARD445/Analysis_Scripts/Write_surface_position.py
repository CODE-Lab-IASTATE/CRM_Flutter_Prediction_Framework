...
Minf = 0.954  # Mach number corresponding to the experimental results for rho = 0.000123 slugs/ft^3
kred = 0.065  # reduced frequency=omega*b/V (in FUN3D, the nondimensional V is equal to mach number)
...
Cref = 0.5*(1.833 + 0.66*1.833) # For the AGARD wing, this is the mean aerodynamic chord
...
for i in range(numEigenVector):
    ...
    for j in range(len(scalingFunction)): # write surface position for each time step
        if j<10:
            ...
            # f = open(('%s/Mode%.0f/mesh_motion_00%.0f.dat' % (analysisDir,i+1,j)),'w')
